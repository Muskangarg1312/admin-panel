...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">Add Bank Entry</h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="#">Bank Entries</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Add Bank Entry</li>
                </ol>
            </nav>
        </div>
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        ?>
                        <form class="form-sample" method="post" action="<?= base_url(); ?>add-bank-entry" id="entryForm">
                                        ...
                                        <div class="">
                                            <select name="parent_bank_id" class="form-control select--category select2">
                                                ...
                                                <?php foreach ($banks as $bank): ?>
                                                   <option data-name='<?= $bank['name'] ?>' value="<?= $bank['id'] ?>" <?= (old('parent_bank_id') == $bank['id']) ? 'selected' : '' ?>><?= $bank['name'] ?></option>
                                               <?php endforeach; ?>
                                           </select>
                                       </div>
                                ...
                                <div class="row ">
                                    <div class="col-md-12 ">
                                        <div class="form-group1">
                                            <label class=" col-form-label label-with-asterisk payment">Mode <span class="text-danger">*</span></label>
                                            <div class="row">
                                                <div class="col-sm-6">
                                                    <div class="form-check">
                                                        <label class="form-check-label"> <input type="radio" class="form-check-input" name="mode" id="membershipRadios1" value="NEFT" <?= (old('mode') == 'NEFT') ? 'checked' : '' ?> /> NEFT</label>
                                                    </div>
                                                </div>
                                                <div class="col-sm-6">
                                                    <div class="form-check">
                                                        <label class="form-check-label"> <input type="radio" class="form-check-input" name="mode" id="membershipRadios2"  value="Transfer" <?= (old('mode') == 'Transfer') ? 'checked' : '' ?> /> Transfer </label>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div >
                         <table id="myNewTable" style="width: 100%;">
                            <tr class="row" style="height: 120px!important;">
                                <td class="col-md-6" >
                                    <div class="links">
                                        <div class="form-group1 ">
                                            <label class=" col-form-label label-with-asterisk">Select Supplier <span class="text-danger">*</span></label>
                                            <div class="">
                                                <select name="supplier_id" class="form-control select--supplier select2">
                                                    <option value="" selected="" disabled=""> Select an option</option>
                                                    <?php foreach ($suppliers as $supplier): ?>
                                                        <option data-name='<?= $supplier['name'] ?>' data-json='<?= json_encode($supplier) ?>' value="<?= $supplier['id'] ?>" <?= (old('supplier_id') == $supplier['id']) ? 'selected' : '' ?>><?= $supplier['name'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="col-form-label label-with-asterisk">Total Value <span class="text-danger">*</span></label>
                                <input type="text" class="form-control select--value bg-white" name="total_value" value="<?= old('total_value') ?> " readonly/>
...
<script type="text/javascript">
 ...
 $(document).ready(function(){
     ...
     $(document).on('click', '.add', function(event) {
        ...
        $(currentRow).addClass('row additional--supplier');
            ...
            .done(function(result) {
                ...
                if(result.status) {
                    ...
                    if( status === 'success' ) {
                        ...
                        $('#date').val(dateValue);
                        $this.trigger('reset');
                        $this.find('.additional--supplier').remove();
